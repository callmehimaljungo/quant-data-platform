...
from utils.quant_validators import clean_financial_data, FinancialDataValidator
...
def render_sidebar():
    ...
    page = st.sidebar.radio(
        "Điều hướng",
        ["🏠 Tổng quan", "📈 Chỉ số Rủi ro", "🏢 Phân tích Sector", 
         "💼 Chiến lược Đầu tư", "🔬 Mô hình ML", "⚙️ Cài đặt"]
    )
    ...
    if r2_ready:
        ...
        from dashboard.r2_loader import get_r2_object_last_modified
        r2_time = get_r2_object_last_modified('processed/gold/cache/realtime_metrics.parquet')
        if r2_time:
            last_update = r2_time
            # Convert to local time if needed, but R2 usually returns UTC.
            # We will display as is or convert. 
            # boto3 returns datetime with timezone (usually UTC)
    ...
    # TIMESTAMPS
    if last_update:
        # Normalize timezone to naive for diff calculation or ensure both are aware
        ...
        # Format for display (convert to local flavor if possible, 
        # but simple strftime is safer for mixed environments)
        # Adding fixed offset for VN display if it's UTC
        if last_update.tzinfo:
             # Very simple conversion for display
        ...
        if val_res['is_valid']:
            ...
            with st.sidebar.expander("Chi tiết vấn đề"):
                for err in val_res['errors']:
                    ...
                    st.caption(f"({err.invalid_count} dòng)")
                
                for warn in val_res['warnings']:
                    st.write(f"🟡 **{warn.column}**: {warn.message}")
                    st.caption(f"({warn.invalid_count} dòng)")
    else:
        st.sidebar.warning("⚠️ Chưa load được data")
...
def render_overview():
    ...
    with col3:
        # Volatility is already in PERCENT format (e.g., 32.96 = 32.96%)
        # Filter only extreme outliers (> 500%)
        valid_vol = risk_df[risk_df['volatility'] < 500]['volatility']
        if len(valid_vol) > 0:
            avg_vol = valid_vol.median()
        else:
            avg_vol = risk_df['volatility'].median()
    ...
    with col4:
        ...
        st.metric(
            "Max Drawdown (Median)",
            f"{avg_mdd:.1f}%",
            help="Mức sụt giảm tối đa từ đỉnh (bao gồm lịch sử dài hạn)"
        )
    ...
    with col2:
        ...
        # Derive sector metrics from actual ticker data
    ...
    st.dataframe(format_dataframe(top_10), use_container_width=True, column_config=get_column_config())
...
def render_risk_metrics():
    ...
    with col1:
        st.subheader("📊 Phân phối Biến động (Volatility)")
        fig = px.histogram(
            filtered_df,
            x='volatility',
            nbins=30,
    ...
    with col2:
        st.subheader("📉 Phân phối Sụt giảm Tối đa (Max Drawdown)")
        fig = px.histogram(
            filtered_df,
            x='max_drawdown',
            nbins=30,
...
def render_ml_models():
    ...
    with tab2:
        ...
        if df is not None and len(df) > 0:
                ...
                with st.expander("Xem tất cả Features"):
...
def render_model_results():
    ...
    def calc_metrics(series):
        ...
        # For short periods (<60 days), use non-annualized metrics to avoid unrealistic numbers
        days = len(series)
        if days >= 60:
            ...
            # Non-annualized for short periods
            ann_ret = total_ret  # Just show raw return
            vol = daily_rets.std() * np.sqrt(days)  # Scale to period
            sharpe = (total_ret) / vol if vol > 0 else 0  # Simple Sharpe, no annualization
    ...
    with col_m1:
        st.write("#### 📅 Hiệu suất Toàn thời gian (All-Time)")
        st.dataframe(
            pd.DataFrame(all_time_data).set_index('Strategy'), 
            use_container_width=True,
            column_config=strat_config
        )
        
    with col_m2:
        st.write("#### ⚡ Hiệu suất Gần đây (30 ngày)")
        st.dataframe(
            pd.DataFrame(recent_data).set_index('Strategy'), 
            use_container_width=True,
            column_config=strat_config
        )
    ...
    if selected_strategy and selected_strategy in strategies:
        ...
        if selected_strategy == 'Low-Beta Quality':
            st.markdown("**Logic Chiến lược:** Chọn cổ phiếu có Beta < 1 và chỉ số chất lượng cao (ROE, biên lợi nhuận)")
            ...
            with col1:
                    ...
                    pie_df = df.sort_values('weight', ascending=False)
                    if len(pie_df) > 10:
                        others = pd.DataFrame([{
                            'ticker': 'Others', 
                            'weight': pie_df.iloc[10:]['weight'].sum(),
                            'sector': 'Mixed'
                        }])
                        pie_df = pd.concat([pie_df.head(10), others])
                        
                    fig = px.pie(pie_df, values='weight', names='ticker', 
                                title='Tỷ trọng Danh mục (Top 10)')
            ...
            st.markdown("**Logic Chiến lược:** Luân chuyển ngành dựa trên VIX và chu kỳ kinh tế")
            ...
            st.markdown("**Logic Chiến lược:** Momentum 12-1 - mua top cổ phiếu tăng trưởng (return 12 tháng, bỏ tháng gần nhất)")
        ...
        with st.expander("Xem Dữ liệu Gốc"):
            st.dataframe(format_dataframe(df), use_container_width=True, column_config=get_column_config())
